...
class SklearnMetricsDataset(CustomDataset):
    ...
    def evaluate(self, results, metric='mIoU', logger=None, work_dir=None, **kwargs):
        ...
        # Se il denominatore è 0, MCC è 0
        ...
        # Aggiungiamo i risultati per ogni singola classe (molto utile per 'lesion')
        for i, class_name in enumerate(self.CLASSES):
            report_lines.extend([
                f"--- Dettaglio Classe: {class_name.upper()} ---",
                f"Valori Assoluti (Pixel):",
                f"  Vero Positivo (TP):  {TP[i]:>12}",
                f"  Vero Negativo (TN):  {TN[i]:>12}",
                f"  Falso Positivo (FP): {FP[i]:>12}",
                f"  Falso Negativo (FN): {FN[i]:>12}",
                f"\nMetriche:",
        ...
        # Aggiungiamo le medie (Macro Average)
        report_lines.extend([
            "-"*50,
            f"[MEDIE MACRO (Macro-Average)]",

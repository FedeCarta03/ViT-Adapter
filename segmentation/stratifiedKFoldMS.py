...
def genera_kfold_stratificato_ms_type(percorso_dataset, file_excel, k=5, estensioni_valide=('.jpg', '.jpeg', '.png'), dir_output="fold_ms_type"):
    ...
    # 1. Carichiamo l'Excel e creiamo il dizionario Paziente -> MS Type
    print(f"Lettura del file clinico: {file_excel}...")
    df = pd.read_csv(file_excel, sep = ";") # Usa read_csv se il file è .csv
    df_pazienti = df.drop_duplicates(subset=['Patient'])
    dizionario_etichette = dict(zip(df_pazienti['Patient'].astype(str), df_pazienti['MS Type'].astype(str)))
    ...
    for p in cartelle_pazienti:
        if p in dizionario_etichette:
            ...
            print(f"  -> Avviso: Paziente {p} saltato (non trovato nel file Excel).")

    if not pazienti_validi:
        print("Errore: Nessun paziente valido per la stratificazione.")
        return

    print(f"Trovati {len(pazienti_validi)} pazienti validi. Inizio la divisione in {k} fold Stratificati per MS Type...")
...
if __name__ == "__main__":
    ...
    FILE_CLINICO = "data_medical/27919209/MSLesSeg_Dataset/MSLesSeg_Dataset/info_dataset/clinical_data.csv" # INSERISCI QUI IL PERCORSO DEL TUO EXCEL
    
    genera_kfold_stratificato_ms_type(
        percorso_dataset=PERCORSO_DATASET, 
        file_excel=FILE_CLINICO,
        k=5, 
        estensioni_valide=('.png', '.jpg', '.jpeg'),
        dir_output="mslesseg_folds_mstype"
    )